...
Use this whenever rules appear to disagree:
...
Use only these verdict terms. Normalize any upstream rejection terminology to them consistently across server responses, UI rendering, specs, and tests.
...
### Audience + Prerequisites (CANONICAL)
These rules are for spec authors, implementers, and reviewers.

- Assume the active phase schema and repo-owned references exist.
- If they are unavailable, do not guess or invent details.
- Record explicit questions or TODOs and keep the artifact in draft until the gap is resolved.

### Key Concepts (POINTER)
Use **Glossary** and **Artifact Primer** for definitions. Do not introduce new requirements here.

### Applicability / Demo Profiles (Actionable Mapping; Skip Rules by Profile)
Pick the closest profile early in Phase 1. Treat only that profile's required modules as active unless later phases explicitly opt in to optional modules.

| Profile | Required | Optional | Not applicable |
| --- | --- | --- | --- |
| **Baseline (text-only)** | presets (apply/run), server-side guardrails, AI seam validation, walkthrough, generated-output + seeded/synthetic labeling, async UX, browser-compatible UI + theme verification, required test tiers | Playwright automation; Long-Running Tasks Workflow | tools, multimodal, assets |
| **Tools** | Baseline + Tools Mode constraints (repo-owned synthetic tool data only), UI-visible tool calls/results, tool mocking strategy/tests | assets, multimodal, Playwright, long-running | none |
| **Multimodal (audio/image)** | Baseline + capture/permission UX state machines, modality allowlists + deterministic validation, feasibility fallbacks, modality-specific tests | tools, assets, Playwright, long-running | none |
| **Assets** | Baseline + repo-owned synthetic assets with UI labeling + automated validation, plus an asset generation plan when needed | tools, multimodal, Playwright, long-running | none |
| **Long-running build work** | optional add-on for any profile when the work is interruption-prone or milestone-driven | see Operational Appendix — Long-Running Tasks Workflow | none |

### Minimal Baseline Path (CANONICAL; Executable Happy Path)
1. **Phase 1 — Define the Boundary (FeatureSpec)**
   Output: a Phase 1 artifact that locks scope to 1-3 headline capabilities with observable acceptance criteria, assumptions/constraints, excluded plumbing, a portable guardrails summary, and a tools-needed vs no-tools decision.
   Gate: acceptance criteria are observable and testable, and the tooling posture is locked.
2. **Phase 2 — Define the Demo Contract (DemoSpec)**
   Output: a Phase 2 artifact that defines the UX contract (views + interaction contracts), shipped presets (apply/run), an in-app walkthrough state machine with step success criteria, guardrails UX semantics using canonical verdict terms, generated vs seeded/synthetic labeling, and traceability to required tests.
   Gate: every headline item is reachable via shipped presets and covered by walkthrough + tests with consistent verdict vocabulary.
3. **Phase 3 — Implement + Prove (CodeSpec/Build)**
   Output: a Phase 3 plan/build that implements the Phase 2 contract with server-side guardrails, AI seam validation, and a concrete, executable test strategy.
   Gate: required mocked-by-default test tiers pass, including presets, guardrails rejects with zero main-model calls, walkthrough, async UX, interaction matrix, theme verification, and AI-seam failure visibility. Live tests run only when explicitly opted in.

### Per-Role Deliverables (by phase)
| Role | Deliverable |
| --- | --- |
| **Spec author** | Publish the active phase artifact with clear, externally observable behavior and traceability anchors. See **Purpose & Output Contract** and **Phase Linkage, Grounding, and Metadata**. |
...
- **Part 1:** phase procedures and core authoring rules
- **Part 2:** cross-cutting invariants and always-on implementation rules
- **Part 3:** optional modules, phase-specific deep dives, and operational appendices
- Use **Canonical Anchors (Quick Index)** to find the single canonical home for any requirement.
- Use checklists, glossary entries, and examples only as aids.
...
1. Confirm scope (1–3 items) and carried exclusions.
2. Confirm grounding and identifier stability.
3. Confirm UX contract plus async and theme behavior.
4. Confirm presets and walkthrough reachability.
5. Confirm server-side guardrails and AI seam behavior.
6. Confirm labeling and privacy requirements.
7. Confirm required tests per the canonical modules.
...
Interpret "walkthrough" as:
- an in-app interactive UI tour,
- one that auto-starts,
- one that can be retriggered and cancelled, and
- one that is implemented as a bounded state machine rather than a presenter script.
...
## Part 2 — Cross-Cutting Build Invariants
## Deterministic Validation & Guardrails (CANONICAL Detail)
**Deterministic validation runs before any model call.** It must cover:

- per-modality MIME/type allowlists and extension sanity,
- max payload sizes,
- modality parsing and decoding checks, including container readability for audio and image inputs,
- audio duration limits and any endpoint-relevant technical constraints that can be validated deterministically, such as channels and sample rate when required by the chosen workflow,
- valid UTF-8 decoding and any required parseability for text-like inputs, such as JSON parseability when the demo accepts JSON-like payloads, and
- an explicit unsupported-modality or unsupported-format short-circuit path.

If deterministic validation fails, return an `unsupported` verdict with a user-visible message and perform zero relevance, safety, and main-model calls.

**Guardrail model responses** must be strict, parseable JSON that conforms exactly to the authoritative guardrail verdict schemas used by the project, with no extra keys. Each response must carry:

1. a machine-checkable decision signal for that guardrail,
2. a non-user-facing reason suitable for audit/debug review, and
3. a user-facing message intended for UI display.

If parsing or validation fails, treat it as a hard failure with a UI-visible, retryable error. Do not infer verdicts from free-form text.

**Guardrail prompts** must:

- use explicit system/developer/user layers,
- include the demo scope/context,
- include an explicit supported-modality declaration and how to interpret provided input summaries,
- include the normalized runtime input summary/metadata plus any optional safe extracted content,
- include an explicit decision policy for that guardrail, covering both relevance and safety expectations, and
- end with an instruction to output JSON only matching the provided schema, with no prose, markdown, or additional keys.
...
**Mocked-by-default tests** must cover allowed, unsupported, and blocked outcomes. Assert that:

1. rejected paths return user-visible reject messages,
2. rejected paths trigger zero main-model calls, and
3. deterministic validation failures hard short-circuit and skip all guardrail model calls.
...
**When to apply:** treat this section as an always-on checklist whenever:

- setting up a new demo that calls OpenAI,
- adding a new OpenAI call path, including guardrails, main generation/extraction, agent/tool-loop, or realtime, or
- refactoring configuration/clients in a way that could reintroduce hardcoded model choices.

**Default model configuration (CANONICAL, centralized):**
- Use `gpt-5.2` with low reasoning effort for Responses API and Agents SDK calls.
- Use `gpt-realtime` for Realtime calls.
- Centralize these defaults in one shared configuration location with exactly one per-demo override point.
- Ensure every OpenAI call path reads model defaults from that shared configuration. Do not hardcode model or reasoning choices at call sites.

Expose overrides via documented environment variables, preferred for deployment flexibility, or a single demo-owned config constant mechanism. Do not add secondary override mechanisms such as per-request overrides, scattered feature-level model picks, or multiple config modules that would undermine the single override point. Ensure the repo-owned example environment file makes every supported override reviewer-visible.

**Model default tests (CANONICAL):** add deterministic tests asserting:

1. defaults resolve to the documented default models and reasoning setting when no overrides are set, and
2. the specific override mechanism the demo actually implements works end-to-end using env patching/config injection.

Tests must not rely on the ambient environment. Do not claim support for an override path unless that exact path is exercised by tests. Also provide a single, reviewer-auditable summary of the effective model configuration so all OpenAI call paths are demonstrably reading from the same centralized defaults.

## OpenAI Credentials (CANONICAL, Env-First + Single Loader)
Use one shared loader for the OpenAI API key.

- Load the key via the framework's standard env-loading mechanism into one shared config module/class used by all OpenAI client code.
- Avoid bespoke per-feature loaders.
- Normalize the configured key before validation, for example by trimming whitespace.
- Validate that the key is present and non-empty at startup or before the first OpenAI API call.

On missing or empty keys, fail fast with a clear, catchable error and, when applicable, a structured HTTP/UI error response whose message names the required API-key environment variable and how to fix it. Do not return a generic 500. Keep the error policy and response shape consistent.

**Documentation deliverables (required):**
- provide a repo-owned example environment file,
- provide a README `Environment Setup` section stating where the env file is discovered from, how to copy and set the key, and at least one run command, and
- never commit real secrets; env files must be gitignored.

**Tests (required, env-patched):** add deterministic tests that:

1. when a key is configured, the loader returns the exact configured value, and
2. when missing or empty, the loader throws/returns a catchable error that names the required variable and yields the documented UX/HTTP failure behavior.

Tests must not rely on an ambient env file.

## Pricing / Cost Estimates (CANONICAL)
When selecting models or estimating run cost:

- use the repo-owned pricing reference document as the single price authority,
- treat prices as USD per 1M tokens,
- structure estimates as a clear breakdown of input, cached input when supported and explicitly used, and output,
- treat reasoning tokens as output and state uncertainty when reasoning-token usage is not directly observable,
- provide a conservative range when token counts are unknown and, unless the spec can justify otherwise, assume output tokens are materially higher than input,
- report realtime and audio pricing as distinct line items per call type,
- map each call path to the correct pricing tier so mixed-modality demos do not misprice costs,
- assume no caching unless the implementation explicitly uses cached input, and
- keep mirrored pricing references in sync to avoid drift.

## Live Test Run Commands (CANONICAL)
Maintain a single repo-owned place that lists:

- copy-pastable commands for the default mocked-by-default test tier(s),
- copy-pastable commands for the opt-in live tier,
- required environment variables and flags, and
- any safety or cost constraints.

Keep this documentation consistent with the Phase 3 testing strategy.
...
**Canonicalization / De-Dup Policy (CANONICAL):**
- Keep exactly one canonical, testable rule per topic.
- Before adding new rules, search **Canonical Anchors (Quick Index; Primary Navigation)** for an existing home and add only a POINTER, or blank redundancy, when one exists.
- When blanking text, never remove the only statement of a requirement.
- Any new CANONICAL rule must include an artifact, UX, and/or test anchor cue.

**Index Consistency (CANONICAL):**
- When adding, removing, or migrating canonical anchors, update **Canonical Anchors (Quick Index; Primary Navigation)** to match.
- Indexes must not conflict with canonical rule text and must defer to precedence. See **How to Interpret Rules: Rule Types, Precedence, and Verdict Terms**.

## Editing Guidance and De-Dup Checklist (CANONICAL)
Use this checklist whenever editing the rules document:

1. Search **Canonical Anchors (Quick Index; Primary Navigation)** for an existing home before adding a new rule.
2. Keep exactly one canonical, testable rule per topic.
3. Add a POINTER, or blank redundancy, instead of duplicating a requirement that already has a canonical home.
4. Do not remove the only statement of a requirement when simplifying or blanking text.
5. Update **Canonical Anchors (Quick Index; Primary Navigation)** whenever canonical anchors move.
6. Use section names or topic anchors, not line numbers, when cross-referencing rules.
...
## Part 3 — Extended Modules, Phase Deep-Dives, and Appendices
...
For every headline demo item, the DemoSpec must:

- make the AI/technical innovation the central user value,
- explicitly identify the moment where AI behavior delivers the core outcome,
- keep non-AI steps minimal and clearly supporting, and
- ensure the walkthrough and success signals foreground this AI moment rather than peripheral UI or plumbing.

## Phase 2 Authoring: Runtime Input + Guardrails Summary Must Be Human-Auditable (CANONICAL)
In addition to defining the guardrails pipeline behavior, the DemoSpec must include a coherent, human-auditable summary of runtime inputs and guardrails covering:

- supported input modalities and how users provide them,
- the end-to-end decision pipeline in plain language,
- the user-visible outcomes for `unsupported`, `blocked`, and `allowed` cases, and
- cancel and retry semantics.

Reviewers must be able to understand the full reject/allow behavior without inferring it from scattered details.

## Phase 2 Demo Narrative + Core Flow (CANONICAL)
In addition to the in-app walkthrough state machine, the DemoSpec must include a **human-readable demo narrative** that Phase 3 can implement without inference. It must include:

1. a concise demo overview a reviewer can read quickly, covering what the user accomplishes, what the AI produces/optimizes, and the observable outcome,
2. an explicit statement that the demo is an in-app product showcase, not a presenter script,
3. an ordered list of core flow steps with observable success signals, and
4. concise example UI copy where helpful, such as titles, empty states, button microcopy, and error/help text.

This narrative must stay consistent with the walkthrough, presets, and acceptance criteria, and must not introduce new requirements beyond Phase 1 scope.

## Phase 2 Scope Lists (CANONICAL; Reviewable In/Out)
Phase 2 must present a concrete, reviewable scope statement as two lists:

- what the demo explicitly includes, and
- what it explicitly excludes.

Use these lists to keep reviews and implementation disciplined, and ensure Phase 3 carries exclusions forward as non-goals and omits implementation work outside the intended demo boundary.
...
Default invariants, which must be true and testable, are:

- walkthrough auto-starts on first launch and is retriggerable,
- the demo accepts runtime inputs, not presets-only,
- presets go through the same server-side deterministic validation + guardrails pipeline as user-entered inputs,
- the intended demo surface is a minimalist, browser-compatible UI unless explicitly overridden, and
- dark/light theme support is enabled and verified.

See **Walkthrough**, **Runtime Input Guardrails Applicability**, **Presets (Semantics)**, **Guardrails Pipeline**, **Browser-Compatible UI Default (Global)**, and **Dark/Light Theme Verification**. Any deviation must be explicitly justified and covered by tests.

## Tools Mode + Synthetic Data Constraint (CANONICAL)
When tools are used, Phase 2 must make an explicit binary decision:

- either the demo uses no tools, or
- it uses tools backed only by synthetic, repo-owned, deterministic data.

Default posture: do not introduce tools unless they are absolutely necessary to demonstrate the in-scope headline capabilities. If the capability can be shown without tools, choose no-tools and keep the demo simpler. Tool calls and results must be UI-visible, reproducible under mocking, and must not depend on ad-hoc external services or live third-party data. If the demo uses no tools, state that clearly to prevent accidental tool creep in Phase 3.

## Phase 3 CodeSpec: Implementation Plan Inventory (CANONICAL)
Phase 3 must produce a reviewable implementation plan inventory that realizes the DemoSpec without adding product scope and keeps the AI value central. It must cover:

- platform posture,
- default mocking posture,
- major components,
- key state and state transitions, including async and error handling,
- minimalist layout/visibility constraints, including generated and seeded/synthetic labeling placement,
- walkthrough implementation, robust highlight targeting, and target-missing fallback,
- presets, synthetic data, and required-assets approach, and
- explicit non-goals derived from excluded plumbing and Phase 2 out-of-scope lists.

## Phase 3 Carry-Forward Obligations (CANONICAL; Phase 2 → Phase 3 Checklist)
Treat Phase 3 as a structured projection of Phase 2 decisions. Phase 3 must explicitly carry forward:

1. the in-scope/out-of-scope scope lists into non-goals,
2. interaction requirements into API coverage confirmations, including voice/tool-loop needs,
3. interaction contracts into an interaction test matrix and control-level loading/enablement behavior,
4. walkthrough steps into walkthrough implementation notes and walkthrough tests,
5. presets and required assets into deterministic fixtures, validation, and tests,
6. guardrails UX semantics into server-side enforcement and rejected-path tests, and
7. when tools are used, the Phase 2 tooling decision into an implementation-ready tooling plan, including UI-visible tool call/result surfaces and mocking strategy.

Any mismatch must be documented as a deliberate deviation.
...
Phase 3 must present the test strategy as a module-by-module coverage plan with concrete verification steps and copy-pastable commands. It must state the default mocked-by-default posture versus any opt-in live tier, including:

- how each AI/tool call is mocked,
- how any intentional delay/timeout/error simulation is mocked,
- what each tier verifies, and
- how to run each tier without reading code.

Include coverage for:

- guardrails short-circuit, including zero main-model calls on reject,
- AI seam request/response validation and parsing failures,
- async state transitions and loading/timeout UX,
- walkthrough state machine and highlight targeting,
- presets integration behavior, and
- tooling behavior, or a no-tools confirmation.

The plan must be auditable and map back to headline demo items.
...
In addition to the walkthrough state machine mechanics, each walkthrough step must state:

1. what the AI is doing at that step, or explicitly that the step is non-AI, and
2. step-level success criteria that are verifiable from the UI.

Walkthrough tests must trace to these step success criteria, not only to navigation mechanics.

## Synthetic Demo Data Justification (CANONICAL)
When Phase 2 uses seeded or synthetic demo data, include a short justification covering:

- which headline demo items it exercises,
- why it is suitable for demonstrating those behaviors, and
- safety/realism notes confirming the data is non-PII, non-sensitive, and bounded.

## Preset Traceability Mapping (CANONICAL)
Phase 2 must map each shipped preset to:

- the headline demo item(s) it exercises, and
- the walkthrough step(s) it supports, including rejected paths when applicable.

This mapping must stay consistent with preset coverage claims and preset integration tests.

## AI/Non-AI Delineation: Where Innovation Lives (CANONICAL)
Phase 2 must explicitly separate AI-driven behavior from deterministic/non-AI behavior and state where the technical innovation lives. Keep this consistent with the AI moment, tool decisions, guardrails behavior, and planned test coverage.
...
Any OpenAI- or tool-boundary failure must surface as an explicit, user-visible error state with a clear recovery action. Do not swallow failures, fall back to empty/placeholder outputs, or present success when request validation, network/timeout, parsing/validation, or post-processing failed. Tests must prove at least one representative failure mode per call type is visible and retryable.
...
Apply the full server-side guardrails workflow to any user-changeable runtime input that can influence execution, including presets that populate the same surfaces. Treat all such inputs as untrusted and run deterministic validation, relevance, and safety before execution.
...
When Phase 2 declares required assets, each asset entry must state:

1. its purpose,
2. where it is used,
3. the key format/size/duration constraints, and
4. the synthetic labeling shown to the user.

If no assets are required, keep the required-assets list empty and ensure no presets, walkthrough steps, or views depend on unlisted files.

## Images & Vision: Centralized Workflow Modules (CANONICAL)
Implement image and vision calls through centralized, reusable workflow modules shared across the demo. Those workflows must own transport/reuse decisions, detail defaults and escalation, deterministic validation/allowlists, and request/response normalization so specs and tests can treat them as the single source of truth.

## Image/vision Changes: Companion Module Coordination (CANONICAL)
When adding or modifying any image/vision flow, keep it aligned with the multimodal rules, allowlists/deterministic validation, AI seam validation, generated-output labeling, model defaults/credentials, and testing standards. Record and test any deliberate exception.

## Phase 3 Interaction Test Matrix: Control-ID Keying (CANONICAL)
Phase 3 must present, or generate from Phase 2, an interaction test matrix keyed by stable control identifiers. Each entry must cover enabled behavior, disabled behavior with user-visible explanation, and loading/in-flight expectations including timeout and recovery, and must stay consistent with walkthrough step success criteria.

## Phase 3 Walkthrough Planning: Step Data + Highlight Strategy (CANONICAL)
Phase 3 must document walkthrough steps as reviewable step definitions, not ad-hoc scripts, and describe a robust highlight-targeting strategy that prefers stable, accessibility-oriented identifiers over brittle DOM assumptions. It must also define fallback behavior for unresolved targets as a retryable error, not a stuck tour.

## Phase 3 AI Dependency Mocking Strategy (CANONICAL)
Phase 3 must define one AI mocking strategy for development and testing that explains how the demo runs deterministically without network calls, what fixtures/snapshots/contract stubs are used, and how mocked outputs stay aligned to the output contracts. Cover guardrails and every main AI call path, including zero-main-model-call proof on rejected paths.

## Phase 3 Request Validation: Debug Logging Policy Statement (CANONICAL)
When Phase 3 describes request preflight validation at the AI seam, it must include a local debug-logging statement that allows only minimal, non-sensitive validation metadata and explicitly forbids API keys, raw prompts, raw model responses, and raw user payloads. Keep it aligned with the global privacy/logging policy.

## Phase 3 Synthetic Data Loading + Deterministic Startup (CANONICAL)
When the demo uses presets or other seeded/synthetic inputs, Phase 3 must specify where that data lives, how it loads deterministically on startup, how the default preset is selected/applied without execution, and how reset/rerun restores the documented baseline state.

## Assets: Conditionality + Naming Discipline (CANONICAL)
Treat assets as strictly conditional: if the required-assets inventory is empty, do not include asset generation steps or runtime dependencies on untracked files. If assets are required, generate/commit/validate them as part of the build and use naming that makes stable identity and synthetic provenance obvious.

## Tooling Decision Lock Across Phases (CANONICAL)
Treat the Phase 1 tools-needed vs no-tools decision as scope-locking. Phase 2 and Phase 3 must implement and test that inherited posture as-is unless a reviewer-auditable deviation updates the Phase 1 decision and propagates consistently through presets, guardrails, walkthrough, and tests.
...
Phase 3 must make stack selection concrete, with no placeholders:

- identify the specific UI, server, and test tools used, or explicitly inherit repo defaults,
- justify any non-default choice, and
- ensure the stack supports the DemoSpec’s UX contract, mocking strategy, and reproducible test runs.

## Phase 3 Scope Discipline: No New Requirements or Acceptance Criteria (CANONICAL)
Phase 3 must not invent new product requirements, acceptance criteria, or user-visible commitments beyond Phase 2 and the active schema. If implementation is blocked by ambiguity, raise a required Phase 2 clarification or record an explicit, reviewable deviation instead of silently adding commitments.
...
Phase 3 must name the structured-output contracts used for relevance and safety decisions and confirm strict parsing/validation against them, with no free-form prose interpretation. Keep this consistent with the two-step guardrails pipeline and canonical verdict terminology.
...
Any time audio behavior changes, explicitly confirm consistency with multimodal capture/permissions/feasibility, allowlists and deterministic validation, async/loading and recovery, credentials/model defaults, AI seam validation, and any per-call opt-in live tests.

## Phase 2 Headline Demo Items: Primary Interaction Mode + Derivation Consistency (CANONICAL)
For each headline demo item in Phase 2, declare its **primary interaction mode**. That declaration must align with interaction requirements, selected API surfaces, applicable multimodal/tools modules, and required tests. If an item is multimodal or tool-assisted, state what is primary versus secondary.

## Phase 2 Interaction Requirements: Deterministic Derivation + Auditable Rationale (CANONICAL)
Phase 2 must present cross-item interaction requirements as a deterministic derivation from the headline demo items, not as discretionary flags. For each requirement, include a short rationale pointing to the item(s) that require it. Resolve any mismatch in Phase 2, not Phase 3.

## Phase 2 Walkthrough Step Inventory: Stable Step Identity + Target Linkage + Retrigger Mechanism (CANONICAL)
Phase 2 must make walkthrough steps reviewable as a stable inventory: each step needs a stable, human-readable identifier, a UI target linkable to the interaction contracts, and consistent label/icon meaning for compact controls. It must also describe the user-visible retrigger mechanism and cover it in walkthrough tests.

## Phase 2 Seed Dataset: Compact, Deterministic, and Justified (CANONICAL)
When the demo uses seeded/synthetic data beyond single-field presets, Phase 2 must include a compact, deterministic, human-scannable seed dataset tied to the headline items it supports. Keep it consistent with seeded/synthetic labeling and reset behavior, cover predictable load/reset in tests, and prefer small structured records over long free-form examples.

## Phase 2 Success Signals + Example Copy: Required Inventories (CANONICAL)
Phase 2 must include:

1. a non-empty inventory of observable success signals aligned to each headline item and core flow step, and
2. a curated inventory of UI copy needed to implement the demo contract without inference.

Keep both inventories consistent with the walkthrough, interaction contracts, presets, and guardrail user-message rules, and do not introduce new scope via copy.

## Phase 2 Tooling Decision Trace: Reviewer-Auditable Carry-Forward (CANONICAL)
When tools are in scope or explicitly out of scope, Phase 2 must carry forward the Phase 1 tooling decision with an explicit consistency assertion. Any deviation must be treated as an explicit cross-phase change that updates the original decision and propagates through presets, walkthrough, guardrails behavior, and tests.

## Pricing-Aware Model Choice and Cost/Quality Trade-offs (CANONICAL)
When a demo makes or changes any OpenAI model choice, include a brief, reviewer-auditable cost/quality rationale grounded in the pricing reference for both guardrails and main feature calls.

- Different cost/quality tiers per call type are acceptable when explicit, compatible with the required structured-output contracts, and covered by mocked-by-default tests.
- If more than one candidate model is plausible, include a compact decision-focused comparison.
- If reasoning-heavy settings are used, acknowledge context-window impact and any mitigation.
- When answering user pricing questions, consult the repo-owned pricing authority and use the same estimate/comparison approach defined in **Pricing / Cost Estimates**.

## Demo Design Decisions: Guardrails + Presets Preservation Contract (CANONICAL)
Treat guardrails and presets as a preservation contract: when changing either area, explicitly reaffirm that the canonical guardrail and preset decisions still hold, or document a deliberate deviation and its rationale.

### When to Apply (CANONICAL)
Apply this preservation contract whenever a change affects runtime input handling, deterministic validation, relevance/safety guardrails, verdict-to-UX messaging, run/submit behavior, preset definitions or UI, preset integration tests, or when model/tool calls happen.

### The Four Non-Negotiable Design Decisions (CANONICAL)
Core decisions to reaffirm:

1. server-side guardrails authority,
2. exactly one relevance check and one safety check with strict structured outputs,
3. a single global preset system where apply populates only and run/submit executes, with no auto-run, and
4. preset coverage plus mocked-by-default preset integration tests as a merge/ship gate.

### Decision Record Requirement (CANONICAL)
When changes could affect these decisions or other cross-cutting demo guarantees, include a lightweight decision record that either reaffirms the preservation contract or declares a deliberate deviation with its scope, rationale, and proving tests.
