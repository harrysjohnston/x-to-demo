...
_GLOBAL_HARD_RULES: tuple[str, ...] = (
    ...
    "Every demo must load OPENAI_API_KEY from environment (use an env file convention for local dev), include an .env.example (or equivalent), and fail fast with a clear UI-visible error message when the key is missing.",
    "CodeSpec must include explicit README setup instructions for env file placement/naming, how to set OPENAI_API_KEY, and how to run locally.",
...
_PHASE_RULES: dict[str, tuple[str, ...]] = {
    ...
    "code_spec": (
        ...
        "Specify runtime configuration for environment-based OPENAI_API_KEY loading: env source precedence, env file convention, required .env.example, and fail-fast UI missing-key behavior.",
        "Include README requirements with exact setup steps, env-file instructions, local run steps, and troubleshooting for missing OPENAI_API_KEY.",
...
_PHASE_PRIORITY_CHECKLIST: dict[str, tuple[str, ...]] = {
    ...
    "code_spec": (
        ...
        "Include runtime_configuration requiring OPENAI_API_KEY via environment, env file convention, .env.example (or equivalent), and clear UI fail-fast behavior when missing.",
        "Include readme_requirements with explicit env-file setup, local run instructions, and missing-key troubleshooting.",
